...
def Vbound():
    try:
        resp = Sdwan.get_request(api = "device/control/networksummary?state=up")
        data = resp.json()
        dataa = data['data']
        k=''
        for x in dataa:
        	k=k+str(x['host-name'])+','+str(x['reachability'])+','+str(x['system-ip'])+','+str(x['site-id'])+','+str(x['device-model'])+','+str(x['controlConnectionsToVsmarts'])+','+str(x['lastupdated'])+';'
        print(k[:-1])
    except:
        print("Wrong")
        sys.exit()
