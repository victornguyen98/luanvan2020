...
def Wan_edge_Health():
    try:
        resp = Sdwan.get_request(api = "alarms/notviewed?state=active")       
        data = resp.json()
        cars = data['data']
        k=''
        for x in cars:
          k = k + str(x['rule_name_display']) + ','+ str(x['values'][0]['system-ip'])+ ','+ str(x['values'][0]['host-name'])+ ','+ str(x['entry_time'])+ ','+ str(x['severity']) + ';'
        print(k[:-1])
    except:
        print("Wrong")
        sys.exit()
