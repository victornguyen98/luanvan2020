...
def Wan_edge_Health():
    try:
        resp = Sdwan.get_request(api = "client/activity/summary")
        data = resp.json()
        cars = data['data']
        k=''
        for x in cars:
          k = k + str(x['name'])+ ','+ str(x['total']) + ','+ str(x['startTime'])+ ','+ str(x['userSessionUserName'])+ ','+ str(x['userSessionIP'])+ ','+ str(x['status']) + ';'
        print(k[:-1])
    except:
        print("Wrong")
        sys.exit()
