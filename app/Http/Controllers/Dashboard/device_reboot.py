...
def Vbound():
    try:
        resp = Sdwan.get_request(api = "device/action/reboot/devices/vedge?groupId=all")
        data = resp.json()
        dataa = data['data']
        k=''
        for x in dataa:
          k=k+str(x.get('host-name',''))+','+str(x.get('system-ip',''))+','+str(x.get('uuid',''))+','+str(x.get('site-id',''))+','+str(x.get('device-model',''))+','+str(x.get('reachability',''))+','+str(x.get('uptime-date',''))+';'
        print(k[:-1])
    except:
        print("Wrong")
        sys.exit()
