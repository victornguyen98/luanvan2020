...
def Wan_edge_Health():
    try:
        resp = Sdwan.get_request(api = "template/device")
        data = resp.json()
        dataa = data['data']
        k=''
        for x in dataa:
          k=k+str(x.get('templateDescription',''))+','+str(x.get('templateDescription',''))+','+str(x.get('deviceType',''))+','+str(x.get('templateAttached',''))+','+str(x.get('devicesAttached',''))+','+str(x.get('lastUpdatedBy',''))+','+str(x.get('lastUpdatedOn',''))+';'
        print(k[:-1])
    except:
        print("Wrong")
        sys.exit()
