...
def Wan_edge_Health():
    try:
        resp = Sdwan.get_request(api = "device/vedgeinventory/detail?status=authorized")
        data = resp.json()
        dataa = data['data']
        k=''
        for x in dataa:
          k=k+str(x.get('host-name',''))+','+str(x.get('system-ip',''))+','+str(x.get('site-id',''))+','+str(x.get('validity',''))+','+str(x.get('chasisNumber',''))+','+str(x.get('serialNumber',''))+';'
        print(k[:-1])
    except:
        print("Wrong")
        sys.exit()
