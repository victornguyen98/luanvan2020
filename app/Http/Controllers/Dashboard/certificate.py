...
def Wan_edge_Health():
    try:
        resp = Sdwan.get_request(api = "certificate/vedge/list")
        data = resp.json()
        dataa = data['data']
        k=''
        for x in dataa:
          k=k+str(x.get('vedgeCertificateState',''))+','+str(x.get('deviceModel',''))+','+str(x.get('chasisNumber',''))+','+str(x.get('host-name',''))+','+str(x.get('deviceIP',''))+','+str(x.get('serialNumber',''))+','+str(x.get('validity',''))+';'
        print(k[:-1])
    except:
        print("Wrong")
        sys.exit()
