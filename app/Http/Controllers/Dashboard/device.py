...
def Wan_edge_Health():
    try:
        resp = Sdwan.get_request(api = "system/device/vedges")
        data = resp.json()
        dataa = data['data']
        k=''
        for x in dataa:
            k=k+str(x['vedgeCertificateState'])+','+str(x['deviceModel'])+','+str(x['chasisNumber'])+','+str(x['serialNumber'])+','+str(x['hardwareCertSerialNumber'])+','+str(x['expirationDate'])+','+str(x.get('host-name',''))+';'
        print(k)
    except:
        print("Wrong")
        sys.exit()
