...
def Wan_Edge_Inven():
    try:
        resp = Sdwan.get_request(api = "device/vedgeinventory/summary")
        data = resp.json()
        string = str(data['data'][0]['value'])+','+str(data['data'][1]['value'])+','+str(data['data'][2]['value'])+','+str(data['data'][3]['value'])
        print(string)
    except:
        print("Wrong")
        sys.exit()
